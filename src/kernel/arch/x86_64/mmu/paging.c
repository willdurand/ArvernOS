...
#define MMU_TRACE_PAGE_ENTRY(message, e)                                       \
  MMU_TRACE("%s "                                                              \
...
void remap_kernel(multiboot_info_t* mbi)
{
  ...
  MMU_TRACE("backup_addr=%p backup_frame=%" PRIu64,
            backup_addr,
  ...
  if (entry->common.framebuffer_addr != 0xB8000) {
    ...
    MMU_TRACE("%s", "mapped VBE framebuffer!");
  }
  ...
  MMU_TRACE("%s", "mapping kernel sections");
  ...
  MMU_TRACE("%s", "kernel sections mapped!");

  MMU_TRACE("mapping multiboot info: start_frame=%" PRIu64
            " end_frame=%" PRIu64,
  ...
  MMU_TRACE("%s", "multiboot info mapped!");
  ...
  MMU_TRACE("mapping multiboot modules: start_frame=%" PRIu64
            " end_frame=%" PRIu64,
            modules_start_frame,
            modules_end_frame);
  ...
  MMU_TRACE("%s", "mapped multiboot modules!");

  // Bitmap for allocated frames.
  MMU_TRACE("mapping %" PRIu64
  ...
  MMU_TRACE("%s", "pages for frame allocator mapped!");
  ...
  paging_set_entry(&p4_table->entries[511],
                   ...
                   PAGING_FLAG_PRESENT | PAGING_FLAG_WRITABLE);
  write_cr3(read_cr3());
  MMU_TRACE("%s", "restored recursive mapping");

  unmap(temporary_page);

  write_cr3(inactive_page_table_frame);
  MMU_TRACE("%s", "switched to new table");
  ...
  MMU_TRACE("guard page at %p", page_start_address(old_page_table));
}
...
void enable_write_protection()
{
  ...
  MMU_TRACE("%s", "enabled write protection (cr0)");
}
...
page_table_t* next_table_address(page_table_t* table, uint64_t index)
{
  if (table->entries[index].present != 1) {
    MMU_TRACE("entry at index=%" PRIu64 " in table=%p is not present, "
              "returning 0",
              index,
              table);
    return 0;
  ...
  if (table->entries[index].huge_page == 1) {
    MMU_TRACE("huge page detected for table=%p index=%" PRIu64 ", returning 0",
              table,
              index);
    return 0;
  }
  ...
  MMU_TRACE("addr=%p, next table=%p", (uint64_t)table, next_table);
...
opt_uint64_t translate_page(page_number_t page_number)
{
  ...
  if (p3) {
    ...
    MMU_TRACE("did not find p3 (%p), returning no value", p3);
  ...
  if (p2) {
    ...
    MMU_TRACE("did not find p2 (%p), returning no value", p2);
    return (opt_uint64_t){ .has_value = false, .value = 0 };
  }
  ...
  if (p1 == 0) {
    MMU_TRACE("did not find p1 (%p), returning no value", p1);
    return (opt_uint64_t){ .has_value = false, .value = 0 };
  }
...
opt_uint64_t pointed_frame(page_entry_t entry)
{
  MMU_TRACE_PAGE_ENTRY("pointed_frame", entry);
...
void map_page_to_frame(page_number_t page_number,
                       ...
                       uint64_t flags)
{
  MMU_TRACE("mapping page=%" PRIu64 " (start_addr=%p) to frame=%" PRIu64
  ...
  if (maybe_frame.has_value) {
    MMU_TRACE("page=%" PRIu64 " already mapped to frame=%" PRIu64,
              page_number,
              maybe_frame.value);
    return;
  }
  ...
  MMU_TRACE("p1_idx=%" PRIu64 " p2_idx=%" PRIu64 " p3_idx=%" PRIu64
            " p4_idx=%" PRIu64,
            p1_idx,
            p2_idx,
            p3_idx,
  ...
  MMU_TRACE("p4=%p", get_p4());

  MMU_TRACE("%s", "get or create p3");
  page_table_t* p3 = next_table_create(get_p4(), p4_idx);
  MMU_TRACE("p3=%p", p3);

  MMU_TRACE("%s", "get or create p2");
  page_table_t* p2 = next_table_create(p3, p3_idx);
  MMU_TRACE("p2=%p", p2);

  MMU_TRACE("%s", "get or create p1");
  ...
  MMU_TRACE("mapped page=%" PRIu64 " to frame=%" PRIu64 " (addr=%p)",
            page_number,
            frame_containing_address(frame),
            frame);
}
...
page_table_t* next_table_create(page_table_t* table, uint64_t index)
{
  MMU_TRACE("table=%p index=%" PRIu64, table, index);
  ...
  if (entry->present) {
    MMU_TRACE("page entry at index=%" PRIu64 " is present", index);
  } else {
    MMU_TRACE("page entry at index=%" PRIu64 " is not present, creating entry",
              index);
    ...
    MMU_TRACE_PAGE_ENTRY("created", table->entries[index]);

    was_created = true;
  }

  page_table_t* next_table = next_table_address(table, index);
  MMU_TRACE("next table=%p", next_table);

  if (was_created) {
    zero_table(next_table);
    MMU_TRACE("zero'ed table=%p", next_table);
  }
...
void paging_set_entry(page_entry_t* entry, uint64_t addr, uint64_t flags)
{
  ...
  MMU_TRACE_PAGE_ENTRY("set", (*entry));
}
...
{
  MMU_TRACE("setting p4 to addr=%p", table);
...
void unmap(page_number_t page_number)
{
  ...
  MMU_TRACE("unmapping page_number=%" PRIu64 " addr=%p", page_number, addr);
  ...
  if (!translate(addr).has_value) {
    MMU_TRACE("cannot unmap page=%" PRIu64 " (start_address=%p) because it is "
              "not mapped",
              page_number,
              addr);
    return;
  ...
  MMU_TRACE_PAGE_ENTRY("clearing", entry);
  ...
  MMU_TRACE("unmapped page=%" PRIu64 " addr=%p", page_number, addr);
}
...
void map_multiple(uint64_t start_page_number,
                  ...
                  uint64_t flags)
{
  MMU_TRACE("mapping %ld pages, starting at page=%" PRIu64,
            number_of_pages,
            start_page_number);
...
{
  MMU_TRACE("unmapping %ld pages, starting at page=%" PRIu64,
            number_of_pages,
            start_page_number);
