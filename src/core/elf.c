...
elf_section_header_t* get_elf_section(elf_header_t* header,
                                      int index);
...
elf_header_t* elf_load(uint8_t* data) {
    ...
    // We need two phases to load all the sections before loading the program code. For now we only do one.
    for (uint16_t i = 0; i < elf->sh_num; i++) {
        ...
        if (section->flags & ELF_SECTION_FLAG_ALLOC && section->size > 0) {
            ...
            uint32_t number_of_pages = paging_amount_for_byte_size(section->addr, section->size);
            ...
            if (section->type == ELF_SECTION_TYPE_PROGBITS) {
                ...
                memcpy((void*)section->addr, (void*)((uint64_t)elf + section->offset),
                       section->size);
...
char* elf_str_table(elf_header_t* header) {
    ...
    return (char*)((uint64_t)header + get_elf_section(header, header->strtab_index)->offset);
}
